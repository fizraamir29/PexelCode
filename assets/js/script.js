...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function showNotification(message, type = "info") {
    ...
    notification.style.cssText = `
        ...
        top: 20px;
        right: 20px;
        background: ${type === "success" ? "linear-gradient(45deg, #10b981, #34d399)" : "linear-gradient(45deg, #7a7780ff, #EC4899)"};
        color: white;
        padding: 15px 25px;
        border-radius: 10px;
        z-index: 10000;
  ...
  window.addEventListener("scroll", () => {
    if (window.scrollY > 100) {
      ...
      header.style.borderBottom = "1px solid rgba(248, 248, 248, 0.3)"
    } else {
      header.style.background = "#123a8aff"
...
document.addEventListener("mousemove", (e) => {
  ...
  if (!cursor) {
    ...
    cursorEl.style.cssText = `
      ...
      width: 20px;
      height: 20px;
      background: linear-gradient(45deg, #0ef1f1ff, #b92871ff);
      border-radius: 50%;
      pointer-events: none;
      z-index: 9999;
      opacity: 0.7;
